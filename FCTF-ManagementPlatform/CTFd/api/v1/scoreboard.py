...
@scoreboard_namespace.route("/fastest_submissions/<int:count>")
@scoreboard_namespace.param("count", "How many fastest submissions to return")

class FastestSubmissions(Resource): 
    @check_account_visibility
    @check_score_visibility
    @cache.cached(timeout=60, key_prefix=make_cache_key)
    def get(self, count):
        ...
        bracket_id = request.args.get("bracket_id")
        standings= get_standings(count=count, bracket_id=bracket_id)
        response = [None] * (len(standings) + 1)  # Initialize with the correct size
        ...
        for i, team in enumerate(standings):
            response[i + 1] = {
                "id": team.account_id,
                "account_url": generate_account_url(account_id=team.account_id),
                "name": team.name,
                "score": int(team.score),
                "bracket_id": team.bracket_id,
                "bracket_name": team.bracket_name,
                "solves": solves_mapper.get(team.account_id, []),
            }
